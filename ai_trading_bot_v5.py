...
Features:
  ...
  • Groq AI signal analysis (llama3-8b-8192)
  • Twelve Data market data (yfinance fallback)
...
try:
    ...
    print("WARNING: groq not installed — pip install groq")
...
GROQ_API_KEY        = os.getenv("GROQ_API_KEY",        "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")
TELEGRAM_BOT_TOKEN  = os.getenv("TELEGRAM_BOT_TOKEN",  "8885360577:AAGDPeUn2drVU1RLNDGJZ91azqMzp0e3QUY")
TELEGRAM_CHAT_ID    = os.getenv("TELEGRAM_CHAT_ID",     "745002829")
TWELVE_DATA_API_KEY = os.getenv("TWELVE_DATA_API_KEY",  "7fbe851a56f44979b799a0c75dcbc546")
...
SYMBOLS: Dict[str, str] = {
    ...
    "XAUUSD": "GC=F",
    "XAGUSD": "SI=F",
    # Indices (yfinance only — Twelve Data futures may need paid plan)
    "US30":   "YM=F",
    "NAS100": "NQ=F",
    "SPX500": "ES=F",
}
...
# ═══════════════════════════════════════════════════════════════════════════════
# RATE LIMITER  (Twelve Data free: 8 calls/min)
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    def wait_if_needed(self):
        with self._lock:
            ...
            if len(self._timestamps) >= self.calls_per_minute:
                wait_sec = 60.0 - (now - self._timestamps[0]) + 0.2
                if wait_sec > 0:
                    log.debug(f"  Rate limit: sleeping {wait_sec:.1f}s")
                    time.sleep(wait_sec)
            self._timestamps.append(time.time())

twelve_rate_limiter = RateLimiter(calls_per_minute=8)
...
class TelegramNotifier:
    ...
    @staticmethod
    def format_signal(sig: Dict, ai_analysis: str = "") -> str:
        emoji   = "📈" if sig["signal"] == "BUY" else "📉"
        conf    = sig.get("confidence", 0)
        stars   = "⭐" * (1 + int(conf // 90))  # 1 star ≥80%, 2 stars ≥90%

        msg = (
            f"🚨 <b>SIGNAL — {sig['symbol']}</b> {emoji} {stars}\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"<b>Direction :</b> {sig['signal']}\n"
            f"<b>Confidence:</b> {conf:.0f}%\n"
            f"<b>Entry     :</b> <code>{sig['entry']}</code>\n"
            f"<b>Stop Loss :</b> <code>{sig['sl']}</code>\n"
            f"<b>TP1       :</b> <code>{sig['tp']}</code>\n"
            f"<b>TP2       :</b> <code>{sig['tp2']}</code>\n"
            f"<b>TP3       :</b> <code>{sig['tp3']}</code>\n"
            f"<b>RR Ratio  :</b> 1:{sig.get('rr_ratio', 0):.1f}\n"
            f"<b>Regime    :</b> {sig.get('regime','')}\n"
            f"<b>Session   :</b> {sig.get('session','')}\n"
            f"<b>TF Agree  :</b> {sig.get('timeframes_agree',0)}/{len(TIMEFRAMES)}\n"
            f"<b>Indicators:</b> {sig.get('indicators','')}\n"
        )
        if ai_analysis:
            msg += f"\n🤖 <b>AI Analysis:</b>\n<i>{ai_analysis}</i>\n"
    ...
    @staticmethod
    def send_signal(sig: Dict, ai_analysis: str = "") -> bool:
        return TelegramNotifier.send(TelegramNotifier.format_signal(sig, ai_analysis))

    @staticmethod
    def send_startup() -> bool:
        msg = (
            "🤖 <b>AI Trading Bot v5 — ONLINE</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"Instruments : {len(SYMBOLS)}\n"
            f"Confidence  : ≥{CONFIDENCE_THRESHOLD}%\n"
            f"MTF         : {MIN_TIMEFRAMES_AGREE}/{len(TIMEFRAMES)} TF\n"
            f"Scan every  : {SCAN_INTERVAL_MINUTES} min\n"
            f"Data source : Twelve Data + yfinance\n"
            f"AI engine   : Groq llama3-8b\n"
            f"⏰ {datetime.utcnow().strftime('%Y-%m-%d %H:%M')} UTC"
        )
        return TelegramNotifier.send(msg)

    @staticmethod
    def send_tp_sl(symbol: str, direction: str, result: str, price: float) -> bool:
        emoji = "✅" if result == "WIN" else "❌"
        label = "TP HIT — WIN" if result == "WIN" else "SL HIT — LOSS"
        msg = (
            f"{emoji} <b>{label}</b>\n"
            f"<b>Symbol:</b> {symbol} {direction}\n"
            f"<b>Price :</b> <code>{price:.5g}</code>\n"
...
class GroqAnalyzer:
    ...
    @staticmethod
    def analyze_signal(sig: Dict) -> str:
        if not GROQ_AVAILABLE or groq_client is None:
            return ""
        try:
            prompt = (
                f"You are an expert forex/crypto/commodities trader. "
                f"Give a concise 2-sentence professional assessment of this trade signal:\n\n"
                f"Symbol: {sig['symbol']} | Direction: {sig['signal']} | Confidence: {sig.get('confidence',0):.0f}%\n"
                f"Entry: {sig['entry']} | SL: {sig['sl']} | TP1: {sig['tp']} | TP2: {sig['tp2']}\n"
                f"Market Regime: {sig.get('regime','')} | Session: {sig.get('session','')} | "
                f"TF Agreement: {sig.get('timeframes_agree',0)}/{len(TIMEFRAMES)}\n"
                f"Reasons: {sig.get('reasons','')}\n\n"
                f"Keep it under 60 words. Be direct and professional."
            )
            resp = groq_client.chat.completions.create(
                model=GroqAnalyzer.MODEL,
    ...
    @staticmethod
    def assess_market_context(symbol: str, regime: str, ind: Dict) -> str:
        """Quick market context note — used in GUI logs."""
        if not GROQ_AVAILABLE or groq_client is None:
            return ""
        try:
            prompt = (
                f"In one sentence (max 20 words), describe the current market context for {symbol}: "
                f"Regime={regime}, RSI={ind.get('rsi',50):.0f}, ADX={ind.get('adx',0):.0f}, "
                f"MACD={'bullish' if ind.get('macd',0) > ind.get('macd_signal',0) else 'bearish'}."
            )
            resp = groq_client.chat.completions.create(
                model=GroqAnalyzer.MODEL,
                messages=[{"role": "user", "content": prompt}],
    ...
    def insert_signal(self, sig: Dict) -> int:
        ...
        INSERT INTO signals
          (symbol,signal,entry,tp,tp2,tp3,sl,result,time,probability,confidence,
           ...
           reasons,timeframes_agree,session,indicators,ai_analysis)
        VALUES
          (:symbol,:signal,:entry,:tp,:tp2,:tp3,:sl,:result,:time,:probability,
           :confidence,:regime,:killzone,:trend,:mtf_confirmation,:structure,:fvg,
           :status,:rr_ratio,:reasons,:timeframes_agree,:session,:indicators,:ai_analysis)
        """
        with self._lock, self._conn() as db:
...
# ═══════════════════════════════════════════════════════════════════════════════
# MARKET DATA FETCHER  (Twelve Data primary → yfinance fallback)
# ═══════════════════════════════════════════════════════════════════════════════

class MarketDataFetcher:
    _cache: Dict[str, Any] = {}
    _cache_time: Dict[str, datetime] = {}
    CACHE_TTL = 900   # 15 min — matches scan interval to cut API calls

    # yfinance ticker → Twelve Data symbol
    TWELVE_SYMBOLS: Dict[str, str] = {
        ...
        "GC=F":     "XAU/USD",
        "SI=F":     "XAG/USD",
        # Indices futures — Twelve Data may require paid plan; yfinance fallback used
        # "YM=F": None, "NQ=F": None, "ES=F": None
    }
    ...
    @classmethod
    def _fetch_twelve(cls, td_symbol: str, interval: str,
                      outputsize: int = 200) -> Optional[pd.DataFrame]:
        td_interval = cls.TWELVE_INTERVALS.get(interval)
        if not td_interval:
            return None
        twelve_rate_limiter.wait_if_needed()
        try:
            resp = requests.get(
            ...
            if data.get("status") != "ok" or "values" not in data:
                log.debug(f"Twelve Data [{td_symbol}]: {data.get('message','error')}")
                return None
            values = data["values"]
            if len(values) < 30:
                return None
            ...
            log.debug(f"  Twelve Data OK: {td_symbol} {td_interval} ({len(df)} bars)")
            return df
        except Exception as e:
            log.debug(f"Twelve Data exception [{td_symbol}]: {e}")
            return None

    @classmethod
    def _fetch_yfinance(cls, ticker: str, interval: str,
        ...
        try:
            ...
            log.debug(f"yfinance error [{ticker}] {interval}: {e}")
            return None
    ...
    def get_bars(cls, ticker: str, interval: str, period: str) -> Optional[pd.DataFrame]:
        ...
        df = None
        td_symbol = cls.TWELVE_SYMBOLS.get(ticker)
        if td_symbol:
            df = cls._fetch_twelve(td_symbol, interval)

        if df is None:
            df = cls._fetch_yfinance(ticker, interval, period)
    ...
    def get_current_price(cls, ticker: str) -> Optional[float]:
        ...
        td_symbol = cls.TWELVE_SYMBOLS.get(ticker)
        if td_symbol:
            df = cls._fetch_twelve(td_symbol, "1min", outputsize=2)
...
class IndicatorEngine:
    ...
    def compute(df: pd.DataFrame) -> Optional[Dict]:
        ...
        try:
            ...
            price  = last(c)
            e9     = last(ema9)   or price
            e21    = last(ema21)  or price
            e50    = last(ema50)  or price
            s200   = last(sma200) or price
            rsi_v  = last(rsi)    or 50.0
            atr_v  = last(atr)    or 0.0
            adx_v  = last(adx)    or 0.0
            pdi_v  = last(plus_di)  or 0.0
            mdi_v  = last(minus_di) or 0.0
            bbu    = last(bb_upper) or price * 1.01
            bbl    = last(bb_lower) or price * 0.99
            bbw    = last(bb_width) or 0.0
            macd_v = last(macd_line) or 0.0
            macd_s = last(macd_sig)  or 0.0
            macd_h = last(macd_hist) or 0.0
            vwap_v = last(vwap) or price
            obv_v  = last(obv)  or 0.0
...
class ConfidenceScorer:
    ...
    def score(cls, ind: Dict, regime: str,
        ...
        if e9 > e21 > e50 and price > s200:
            buy_score += ema_wt;  reasons_buy.append("EMA stack bullish (9>21>50 + above SMA200)")
        elif e9 < e21 < e50 and price < s200:
            sell_score += ema_wt; reasons_sell.append("EMA stack bearish (9<21<50 + below SMA200)")
        elif e9 > e21 > e50:
            buy_score += ema_wt * 0.6;  reasons_buy.append("EMA trend bullish (short-term)")
        elif e9 < e21 < e50:
            sell_score += ema_wt * 0.6; reasons_sell.append("EMA trend bearish (short-term)")

        # 2. RSI momentum
        rsi_wt = w.get("rsi_momentum", 15)
        if 50 < rsi < 70:
            buy_score += rsi_wt;        reasons_buy.append(f"RSI bullish momentum ({rsi:.0f})")
        elif 30 < rsi < 50:
            sell_score += rsi_wt;       reasons_sell.append(f"RSI bearish momentum ({rsi:.0f})")
        elif rsi <= 30:
            buy_score += rsi_wt * 0.7;  reasons_buy.append(f"RSI oversold recovery ({rsi:.0f})")
        elif rsi >= 70:
            sell_score += rsi_wt * 0.7; reasons_sell.append(f"RSI overbought ({rsi:.0f})")

        # 3. MACD crossover
        macd_wt = w.get("macd_crossover", 15)
        if macd > ms and mh > 0:
            buy_score += macd_wt;       reasons_buy.append("MACD bullish crossover + positive histogram")
        elif macd < ms and mh < 0:
            sell_score += macd_wt;      reasons_sell.append("MACD bearish crossover + negative histogram")
        elif macd > ms:
        ...
        if adx > 25:
            if pdi > mdi:
                buy_score += adx_wt;   reasons_buy.append(f"Strong trend ADX={adx:.0f} +DI>-DI")
            else:
                sell_score += adx_wt;  reasons_sell.append(f"Strong trend ADX={adx:.0f} -DI>+DI")
        elif adx > 20:
            if pdi > mdi: buy_score += adx_wt * 0.5
            else:         sell_score += adx_wt * 0.5

        # 5. Bollinger Band position
        bb_wt  = w.get("bb_position", 10)
        bb_mid = (bbu + bbl) / 2
        if price > bb_mid and price < bbu * 0.998:
            buy_score += bb_wt;        reasons_buy.append("Price above BB midline, room to upper band")
        elif price < bb_mid and price > bbl * 1.002:
            sell_score += bb_wt;       reasons_sell.append("Price below BB midline, room to lower band")

        # 6. VWAP position
        vwap_wt = w.get("vwap_position", 10)
        if price > vwap * 1.0005:
            buy_score += vwap_wt;      reasons_buy.append("Price above VWAP (institutional bullish bias)")
        elif price < vwap * 0.9995:
            sell_score += vwap_wt;     reasons_sell.append("Price below VWAP (institutional bearish bias)")

        # 7. Support / Resistance
        sr_wt  = w.get("support_resist", 10)
        sr_rng = (res - sup) * 0.1
        if price < sup + sr_rng:
            buy_score += sr_wt;        reasons_buy.append(f"Price at support zone ({sup:.5g})")
        elif price > res - sr_rng:
            sell_score += sr_wt;       reasons_sell.append(f"Price at resistance zone ({res:.5g})")

        # 8. Volume / OBV
        vol_wt = w.get("volume_confirm", 5)
        if obv > obvp * 1.01:
            buy_score += vol_wt;       reasons_buy.append("OBV rising — institutional buying")
        elif obv < obvp * 0.99:
            sell_score += vol_wt;      reasons_sell.append("OBV falling — institutional selling")

        # 9. Trend structure
        ts_wt = w.get("trend_structure", 5)
        if price > e50 and e50 > s200:
            buy_score += ts_wt;        reasons_buy.append("Long-term uptrend structure confirmed")
        elif price < e50 and e50 < s200:
            sell_score += ts_wt;       reasons_sell.append("Long-term downtrend structure confirmed")

        # 10. Regime bonus
        reg_wt = w.get("regime_bonus", 5)
        if regime in ("TRENDING", "BREAKOUT"):
            total_tmp = buy_score + sell_score or 1
            buy_score  += reg_wt * (buy_score  / total_tmp)
            sell_score += reg_wt * (sell_score / total_tmp)
...
class MultiTimeframeAnalyzer:
    ...
    def analyze(symbol: str, ticker: str,
        ...
        for tf_name, (interval, period) in TIMEFRAMES.items():
            ...
            if direction == "BUY":   buy_count  += 1
            elif direction == "SELL": sell_count += 1
...
class RiskManager:
    ...
    def _ensure_today(self):
        ...
        week  = (datetime.utcnow() - timedelta(
                    days=datetime.utcnow().weekday())).strftime("%Y-%m-%d")
        self.db.execute(
            "INSERT OR IGNORE INTO risk_state"
            "(date,daily_loss,weekly_start,weekly_loss,trade_count,last_updated) "
            "VALUES(?,0,?,0,0,?)",
    ...
    def position_size(self, entry: float, sl: float) -> float:
        ...
        if pip_value == 0:
            return 0.01
        return round(min(risk_amount / pip_value, 10.0), 2)
    ...
    def _update_symbol_weights(self):
        ...
        for r in rows:
            ...
            self.db.execute("""
                ...
                VALUES (?,?,?,?,?,?,?,?,?)
            """, (r["symbol"], total, r["wins"], r["losses"],
                  round(wr * 100, 1), round(pf, 2), round(weight, 2),
                  round(r["avg_conf"] or 80, 1), datetime.utcnow().isoformat()))
...
def is_trading_session(filters: Dict) -> bool:
    if not filters.get("killzone", True):
        return True
    return current_session() in ("LONDON_OPEN", "LONDON", "NY_OVERLAP", "NY")
...
class SignalGenerator:
    ...
    def _compute_targets(self, direction: str,
                         ind: Dict) -> Tuple[float, float, float, float, float]:
        price = ind["price"]
    ...
    def process_symbol(self, symbol: str, filters: Dict) -> Optional[Dict]:
        ...
        if direction is None:
            log.debug(f"  {symbol}: MTF disagreement ({tf_agree}/{MIN_TIMEFRAMES_AGREE})")
            return None
        ...
        if direction != direction2:
            log.debug(f"  {symbol}: MTF ({direction}) ≠ H1 ({direction2})")
            return None
        if confidence < CONFIDENCE_THRESHOLD:
            log.debug(f"  {symbol}: confidence {confidence:.0f}% < {CONFIDENCE_THRESHOLD}%")
            return None

        log.info(f"  ✓ {symbol} {direction} conf={confidence:.0f}% regime={regime} TF={tf_agree}")
        ...
        ind_summary = (
            f"RSI={ind['rsi']:.0f} "
            f"MACD={'▲' if ind['macd']>ind['macd_signal'] else '▼'} "
        ...
        # ── Groq AI analysis ─────────────────────────────────────
        ai_text = GroqAnalyzer.analyze_signal(sig)
        sig["ai_analysis"] = ai_text
        if ai_text:
            log.info(f"  🤖 AI: {ai_text[:80]}...")
        ...
        # ── JSON files (GUI compatibility) ────────────────────────
        self._update_json_files(sig)

        # ── Telegram alert ────────────────────────────────────────
        if filters.get("telegram", True):
            TelegramNotifier.send_signal(sig, ai_text)
    ...
    def close_open_positions(self):
        ...
        for sig in opens:
            ...
            if direction == "BUY":
                if price >= tp:
                    self.db.close_signal(symbol, "WIN", profit_pips=abs(tp - entry))
                    self.cooldown.remove(symbol)
                    log.info(f"  TP HIT  {symbol} BUY → WIN  @ {price:.5g}")
                    TelegramNotifier.send_tp_sl(symbol, direction, "WIN", price)
                    closed = True
                elif price <= sl:
                    self.db.close_signal(symbol, "LOSS", profit_pips=-abs(entry - sl))
                    self.cooldown.remove(symbol)
                    self.risk.record_trade_result("LOSS", MAX_RISK_PER_TRADE)
                    log.warning(f"  SL HIT  {symbol} BUY → LOSS @ {price:.5g}")
                    TelegramNotifier.send_tp_sl(symbol, direction, "LOSS", price)
                    closed = True
            else:
                if price <= tp:
                    self.db.close_signal(symbol, "WIN", profit_pips=abs(entry - tp))
                    self.cooldown.remove(symbol)
                    log.info(f"  TP HIT  {symbol} SELL → WIN  @ {price:.5g}")
                    TelegramNotifier.send_tp_sl(symbol, direction, "WIN", price)
                    closed = True
                elif price >= sl:
                    self.db.close_signal(symbol, "LOSS", profit_pips=-abs(sl - entry))
                    self.cooldown.remove(symbol)
                    self.risk.record_trade_result("LOSS", MAX_RISK_PER_TRADE)
                    log.warning(f"  SL HIT  {symbol} SELL → LOSS @ {price:.5g}")
                    TelegramNotifier.send_tp_sl(symbol, direction, "LOSS", price)
                    closed = True
...
def main():
    ...
    log.info(f"  Data source    : Twelve Data + yfinance fallback")
    log.info(f"  AI engine      : Groq {GroqAnalyzer.MODEL}")
    log.info(f"  Telegram       : {'enabled' if TELEGRAM_BOT_TOKEN else 'disabled'}")
    log.info("=" * 60)
    ...
    while True:
        ...
        if not ok:
            log.warning(f"  ⛔ {reason}  —  bot paused")
            TelegramNotifier.send(f"⛔ <b>Risk Limit Active</b>\n{reason}")
        ...
        for symbol in SYMBOLS:
            try:
                ...
                if sig:
                    signals_generated += 1
                    log.info(
                        f"\n  ══ SIGNAL ══  {sig['symbol']} {sig['signal']}\n"
                        f"  Conf={sig['confidence']:.0f}%  Entry={sig['entry']}\n"
                        f"  SL={sig['sl']}  TP1={sig['tp']}  TP2={sig['tp2']}\n"
                        f"  RR=1:{sig['rr_ratio']:.1f}  Regime={sig['regime']}\n"
        ...
        if signals_generated == 0:
            log.info("  No high-quality setups found — waiting for better opportunities")
        else:
...
if __name__ == "__main__":
    try:
        ...
        TelegramNotifier.send("⛔ <b>AI Trading Bot v5 — OFFLINE</b>\nStopped by user.")
